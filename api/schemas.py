...
class Trading(BaseModel):
    """取引条件。worker/engine の trading とキー整合。"""
    balance: Optional[float] = Field(None, gt=0, description="初期残高")
    ...
    margin_call: Optional[float] = Field(None, ge=0)
    lot_mode: Literal["fixed", "dynamic"] = "fixed"
    lot_size: Optional[float] = Field(None, gt=0, le=1000, description="固定ロット時のロット数")
    risk_pct: Optional[float] = Field(None, gt=0, le=100, description="動的ロット時のリスク%")
    pyramid: Optional[bool] = Field(False, description="True: シグナルごとに積み増しエントリー")
...
# === Entry blocks (AND/OR per block) ===
class EntryBlockSchema(BaseModel):
...
class StrategyMvp0(BaseModel):
    ...
    entry_blocks: Optional[List[EntryBlockSchema]] = None  # 指定時はブロック単位の AND/OR を適用
