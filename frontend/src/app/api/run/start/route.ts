...
export async function POST(req: NextRequest) {
  try {
    // 1) 先に cookieStore を作る
    const cookieStore = await cookies();

    // 2) それを渡して SSR クライアント生成
    ...
    let userId: string | null = null;
    {
      ...
      if (ssr?.user) userId = ssr.user.id;
      else {
    ...
    if (userId) {
      const { data: prof, error: perr } = await supaAdmin
        ...
        .eq('id', userId)
        .single();
      if (perr) return NextResponse.json({ error: 'profile_error' }, { status: 500 });
      plan = (prof?.plan ?? 'free') as any;

      if (plan === 'starter') {
        const { data: r, error } = await supaSSR.rpc('reserve_starter_quota', {
          p_idem: idem, p_limit: 30, p_tz: TZ,
        });
        if (error) return NextResponse.json({ error: 'starter_quota_rpc_error' }, { status: 500 });
        if (!r || r.length === 0) return NextResponse.json({ error: 'starter_quota_exceeded' }, { status: 402 });
      } else if (plan === 'free') {
        const { data: r, error } = await supaSSR.rpc('reserve_free_quota', {
          p_idem: idem, p_limit: FREE_LIMIT, p_tz: TZ,
        });
        if (error) return NextResponse.json({ error: 'free_quota_rpc_error' }, { status: 500 });
        if (!r || r.length === 0) return NextResponse.json({ error: 'free_quota_exceeded' }, { status: 402 });
      }
    } else {
      ...
      if (used >= FREE_LIMIT) {
        return NextResponse.json({ error: 'free_quota_exceeded' }, { status: 402 });
      }
      cookieStore.set('gt_guest_runs', JSON.stringify({ d: today, n: used + 1 }), {
        httpOnly: true,
        sameSite: 'lax',
    ...
    // --- FastAPI 実行（※ URL をテンプレートリテラルに）
// app/api/run/start/route.ts の FastAPI 呼び出し部分を置換
...
const apiRes = await fetch(url, {
  ...
  body: JSON.stringify(payload),   // ←← これを追加
  cache: 'no-store',
});


// ←← ここが重要：FastAPIの返答をそのまま返す
