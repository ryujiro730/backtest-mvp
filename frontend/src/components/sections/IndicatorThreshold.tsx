  ...
  return (
    <Card>
      <CardHeader className="flex justify-between items-center">
        <CardTitle>インジケーター条件</CardTitle>

        <div className="flex items-center gap-3">
          <span className="text-sm text-muted-foreground">
            {logic === "AND" ? "すべて満たす" : "いずれか"}
          </span>
          <Switch
            checked={logic === "AND"}
            onCheckedChange={(v) =>
          ...
          {rules.map((r: any) => (
            <AccordionItem key={r.id} value={r.id}>
              <AccordionTrigger className="flex gap-4">
                <span className="font-medium uppercase">{r.indicator}</span>
                <span className="text-muted-foreground">
                  {r.direction === "long" ? "ロング" : "ショート"}
                </span>
              </AccordionTrigger>
...
>
  ...
  <SelectContent>
    ...
    <SelectItem value="sma">SMA（単純移動平均）</SelectItem>
    <SelectItem value="ema">EMA（指数移動平均）</SelectItem>

    <SelectItem value="macd">MACD</SelectItem>
    <SelectItem value="bbands">ボリンジャーバンド</SelectItem>
    <SelectItem value="stoch">ストキャスティクス</SelectItem>

    <SelectItem value="adx">ADX</SelectItem>
    <SelectItem value="cci">CCI</SelectItem>

    <SelectItem value="vwap">VWAP</SelectItem>
    <SelectItem value="supertrend">スーパートレンド</SelectItem>

    <SelectItem value="donchian">ドンチャン</SelectItem>
    <SelectItem value="breakout">ブレイクアウト</SelectItem>
  </SelectContent>
</Select>
